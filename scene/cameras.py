...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy,
        ...
        self.R = torch.tensor(R, device='cuda').float()
        self.T = torch.tensor(T, device='cuda').float()
        ...
        if self.preload_img:
            gt_image, loaded_mask = process_image(self.image_path, self.resolution, ncc_scale)
