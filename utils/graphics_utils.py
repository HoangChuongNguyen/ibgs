...
def depth_map_to_normal(viewpoint_cam, depth, offset=None, normal=None, scale=1):
    ...
    return normal_ref.permute(2, 0, 1)


def depth_to_normal_pgsr(viewpoint_cam, depth, offset=None, normal=None, scale=1):
    intrinsic_matrix, extrinsic_matrix = viewpoint_cam.get_calib_matrix_nerf(scale=scale)
    st = max(int(scale / 2) - 1, 0)
    if offset is not None:
        offset = offset[st::scale, st::scale]
